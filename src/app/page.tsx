  ...
  const menus = [
    ...
    },
    {
      ...
      items: hasPermission('purchase') ? [
        ...
        { header: "Hub" },
        { label: "📈 Purchase Hub (Real-time)", icon: TrendingUp, action: () => setView("purchase-hub") },
        { header: "Suppliers" },
        { label: "Supplier Directory", icon: Users, action: () => setView("supplier-form") },
        { label: "Supplier Payments", icon: DollarSign, action: () => setView("purchase-hub") },
...
function PaymentModal({ total, subtotal, tax, discount, itemCount, invoiceNumber, customerName, customerId, onClose, onComplete }: {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-0 sm:p-4"
      onClick={onClose}
    >
      <motion.div
...
function StockListPopup({ products, searchText, onSelect, onClose, onNew }: {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/40 flex items-start justify-center pt-16 z-[200]"
      onClick={onClose}
    >
      <motion.div
      ...
      >
        ...
        <div className="flex-shrink-0 flex items-center justify-between px-3 h-7 text-white" style={{ backgroundColor: '#5B9BD5' }}>
          ...
          <button aria-label="x" onClick={onClose} className="h-5 w-5 rounded hover:bg-white/25 flex items-center justify-center transition">
            <X className="h-3.5 w-3.5" />
          </button>
        </div>
...
function CartPreviewModal({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-2 sm:p-4"
      onClick={onClose}
    >
      <motion.div
        initial={{ scale: 0.95, y: 30 }}
      ...
      >
        ...
        <div className="flex-shrink-0 bg-gradient-to-r from-violet-600 via-purple-600 to-fuchsia-600 text-white px-3 sm:px-6 py-2.5 sm:py-4 flex items-center justify-between gap-2">
          ...
          <button aria-label="x" onClick={onClose} className="h-8 w-8 sm:h-9 sm:w-9 rounded-lg bg-white/15 hover:bg-white/25 flex items-center justify-center transition flex-shrink-0">
...
function ReceiptModal({ payment, onClose }: { payment: PaymentResult; onClose: () => void }) {
  ...
  return (
    <>
      ...
      {/* EMAIL MODAL */}
      {showEmail && (
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4" onClick={() => setShowEmail(false)}>
      ...
      {/* PRINT OVERLAY */}
      {printMode && (
        <div className="fixed inset-0 z-[200] flex flex-col print-receipt-overlay" style={{ background: '#ffffff !important' }}>
      ...
      {/* WHATSAPP MODAL */}
      {showWhatsApp && (
        <div className="fixed inset-0 z-[200] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-0 sm:p-4" onClick={() => setShowWhatsApp(false)}>
          <div className="dialog-premium shadow-premium-xl w-full max-w-md max-h-[80vh] flex flex-col overflow-hidden" onClick={(e) => e.stopPropagation()}>
...
function StandardCalculator({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.9, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.9, y: 30 }}
        ...
        <div className="gradient-premium-slate text-white px-5 py-3 flex items-center justify-between">
          ...
          <button aria-label="x" onClick={onClose} className="h-7 w-7 rounded-lg bg-white/15 hover:bg-white/25 flex items-center justify-center transition">
            <X className="h-4 w-4" />
          </button>
        </div>
...
function CashCalculator({ total, onClose }: { total: number; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[200] flex items-center justify-center p-0 sm:p-4" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 30 }}
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white px-6 py-4">
          <div className="flex items-center justify-between">
            ...
            <button aria-label="x" onClick={onClose} className="h-8 w-8 rounded-lg bg-white/15 hover:bg-white/25 flex items-center justify-center"><X className="h-4 w-4" /></button>
          </div>
        </div>

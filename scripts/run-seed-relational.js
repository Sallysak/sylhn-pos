...
async function main() {
  ...
  const admin = await p.systemUser.create({ data: { username: "admin", password: adminPwd, fullName: "System Administrator", role: "admin", phone: "+233592766044", email: "admin@sylhn.com", passwordResetRequired: true, permissions: JSON.stringify({ pos: true, sales: true, stock: true, purchase: true, accounts: true, telephone: true, maintenance: true, financeOps: true, canVoid: true, canDiscount: true, canAdjustStock: true, canDeleteProducts: true, canExport: true }) } });
  const manager = await p.systemUser.create({ data: { username: "manager", password: managerPwd, fullName: "Store Manager", role: "manager", phone: "+233 24 111 2222", email: "manager@sylhn.com", passwordResetRequired: true, permissions: JSON.stringify({ pos: true, sales: true, stock: true, purchase: true, accounts: true, telephone: true, maintenance: false, financeOps: true, canVoid: true, canDiscount: true, canAdjustStock: true, canDeleteProducts: false, canExport: true }) } });
  const cashier = await p.systemUser.create({ data: { username: "cashier", password: cashierPwd, fullName: "Sarah Johnson", role: "cashier", phone: "+233 24 333 4444", email: "sarah@sylhn.com", passwordResetRequired: true, permissions: JSON.stringify({ pos: true, sales: true, telephone: true, canDiscount: true }) } });
  await p.systemUser.create({ data: { username: "stockkeeper", password: stockkeeperPwd, fullName: "Kwame Mensah", role: "stockkeeper", phone: "+233 24 555 7777", email: "kwame@sylhn.com", passwordResetRequired: true, permissions: JSON.stringify({ pos: true, stock: true, purchase: true, canAdjustStock: true, canExport: true }) } });
  await p.systemUser.create({ data: { username: "accountant", password: accountantPwd, fullName: "Grace Owusu", role: "accountant", phone: "+233 24 999 8888", email: "grace@sylhn.com", passwordResetRequired: true, permissions: JSON.stringify({ pos: true, accounts: true, financeOps: true, canExport: true }) } });
